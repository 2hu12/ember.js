...
moduleFor('Basic Routing - Decoupled from global resolver', class extends ApplicationTestCase {
  ...
  ["@test The template is not re-rendered when the route's context changes"](assert) {
    ...
    return this.visit('/page/first').then(() => {
      ...
      let router = this.applicationInstance.lookup('router:main');
      return run(() => router.transitionTo('page', EmberObject.create({
        name: 'third'
      })));
    }).then(() => {
  ...
  ['@test Aborting/redirecting the transition in `willTransition` prevents LoadingRoute from being entered'](assert) {
    ...
    return this.visit('/').then(() => {
      ...
      run(router, 'handleURL', '/nork'); // Attempted transitions out of index should redirect to about

      redirect = true;
      run(router, 'transitionTo', 'nork');
      run(router, 'transitionTo', 'index'); // Redirected transitions out of index to a route with a
  ...
  ['@test Components inside an outlet have their didInsertElement hook invoked when the route is displayed'](assert) {
    ...
    this.add('component:my-component', Component.extend({
      didInsertElement() {
        myComponentCounter++;
      }

    }));
    this.add('component:other-component', Component.extend({
      didInsertElement() {
