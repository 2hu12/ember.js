...
import { addObserver, removeObserver, notifyPropertyChange, defineProperty, computed, getCachedValueFor, Mixin, mixin, observer, beginPropertyChanges, endPropertyChanges, changeProperties, get, set } from '..';
...
moduleFor('addObserver', class extends AbstractTestCase {
  ...
  ['@test observer should fire when dependent property is modified'](assert) {
    ...
    defineProperty(obj, 'foo', computed('bar', function () {
  ...
  ['@test observer should continue to fire after dependent properties are accessed'](assert) {
    ...
    defineProperty(obj, 'anotherProp', computed('prop', function () {
  ...
  ['@test observer specified via Function.prototype extensions via brace expansion should fire when dependent property changes'](assert) {
    if (!FUNCTION_PROTOTYPE_EXTENSIONS && ENV.EXTEND_PROTOTYPES.Function) {
      ...
      defineProperty(obj, 'bar', computed('baz', function () {
      ...
      get(obj, 'foo');
      set(obj, 'baz', 'Baz'); // fire once for foo, once for bar
...
moduleFor('removeObserver', class extends AbstractTestCase {
...
let obj, count;
moduleFor('addObserver - dependentkey with chained properties', class extends AbstractTestCase {
  beforeEach() {
    obj = {
      foo: {
        bar: {
  ...
  ['@test depending on a chain with a capitalized first key'](assert) {
    ...
    assert.equal(val, 'BUZZ');
    assert.equal(count, 1);
    set(get(obj, 'Capital.foo.bar'), 'baz', {
      biff: 'BLARG'
    });
    assert.equal(val, 'BLARG');
    assert.equal(count, 2);
    set(get(obj, 'Capital.foo'), 'bar', {
      baz: {
        biff: 'BOOM'
      }
    });
...
moduleFor('props/observer_test - setting identical values', class extends AbstractTestCase {
  ...
  ['@test setting a cached computed property whose value has changed should trigger'](assert) {
    let obj = {};
    defineProperty(obj, 'foo', computed('baz', {
      get: function () {
        return get(this, 'baz');
      },
...
moduleFor('changeProperties', class extends AbstractTestCase {
...
moduleFor('Keys behavior with observers', class extends AbstractTestCase {
