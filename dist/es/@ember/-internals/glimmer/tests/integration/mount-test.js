...
moduleFor('{{mount}} single param assertion', class extends RenderingTestCase {
...
moduleFor('{{mount}} assertions', class extends RenderingTestCase {
...
moduleFor('{{mount}} test', class extends ApplicationTestCase {
  constructor() {
    super(...arguments);
  ...
  ['@test it boots an engine, instantiates its application controller, and renders its application template'](assert) {
    this.engineRegistrations['template:application'] = compile('<h2>Chat here, {{username}}</h2>', {
  ...
  async ['@test it emits a useful backtracking re-render assertion message'](assert) {
    ...
    this.engineRegistrations['template:application'] = compile('hi {{person.name}} [{{component-with-backtracking-set person=person}}]', {
      moduleName: 'my-app/templates/application.hbs'
    });
    this.engineRegistrations['controller:application'] = Controller.extend({
      person: {
    ...
    this.engineRegistrations['template:components/component-with-backtracking-set'] = compile('[component {{person.name}}]', {
  ...
  ['@test it renders with a bound engine name']() {
    ...
    this.add('engine:foo', Engine.extend({
      ...
      init() {
        ...
        this.register('template:application', compile('<h2>Foo Engine</h2>', {
          moduleName: 'my-app/templates/application.hbs'
        }));
      }
    ...
    this.add('engine:bar', Engine.extend({
      ...
      init() {
        ...
        this.register('template:application', compile('<h2>Bar Engine</h2>', {
          moduleName: 'my-app/templates/application.hbs'
        }));
      }
  ...
  ['@test it declares the event dispatcher as a singleton']() {
    ...
    this.add('engine:foo', Engine.extend({
      ...
      init() {
        ...
        this.register('template:application', compile('<h2>Foo Engine: {{tagless-component}}</h2>', {
          moduleName: 'my-app/templates/application.hbs'
        }));
        ...
        this.register('template:components/tagless-component', compile('Tagless Component', {
...
moduleFor('{{mount}} params tests', class extends ApplicationTestCase {
  constructor() {
    super(...arguments);
    this.add('engine:paramEngine', Engine.extend({
      ...
      init() {
        ...
        this.register('template:application', compile('<h2>Param Engine: {{model.foo}}</h2>', {
          moduleName: 'my-app/templates/application.hbs'
        }));
      }
  ...
  ['@test it renders contextual components passed as parameter values']() {
    ...
    this.add('engine:componentParamEngine', Engine.extend({
      ...
      init() {
        ...
        this.register('template:application', compile('{{model.foo}}', {
          moduleName: 'my-app/templates/application.hbs'
        }));
      }
