...
import { EMBER_IMPROVED_INSTRUMENTATION } from '@ember/canary-features'; // IE includes the host name
...
moduleFor('{{link-to}} component (routing tests)', class extends ApplicationTestCase {
  constructor() {
    super();
  ...
  [`@test the {{link-to}} component doesn't apply a 'disabled' class if disabledWhen is not provided`](assert) {
  ...
  [`@test the {{link-to}} component supports a custom disabledClass set via bound param`](assert) {
  ...
  [`@test the {{link-to}} component responds to clicks according to its disabledWhen bound param`](assert) {
  ...
  [`@test The {{link-to}} component supports 'classNameBindings' with custom values [GH #11699]`](assert) {
    this.addTemplate('index', `
        <h3 class="home">Home</h3>
...
moduleFor('{{link-to}} component (routing tests - location hooks)', class extends ApplicationTestCase {
  constructor() {
    super();
    this.updateCount = 0;
    this.replaceCount = 0;
  ...
  ['@test The {{link-to}} component supports URL replacement via replace=boundTruthyThing'](assert) {
    this.addTemplate('index', `
        <h3 class="home">Home</h3>
...
if (EMBER_IMPROVED_INSTRUMENTATION) {
  moduleFor('The {{link-to}} component with EMBER_IMPROVED_INSTRUMENTATION', class extends ApplicationTestCase {
    constructor() {
      super();
      this.router.map(function () {
        this.route('about');
    ...
    ['@test The {{link-to}} component interaction event includes the transition in the after hook'](assert) {
      assert.expect(1);
      subscribe('interaction.link-to', {
        before() {},
...
moduleFor('The {{link-to}} component - nested routes and link-to arguments', class extends ApplicationTestCase {
  ...
  [`@test The {{link-to}} component does not disregard current-when when it is given explicitly for a route`](assert) {
    this.router.map(function () {
      this.route('index', {
        path: '/'
      }, function () {
  ...
  ['@test The {{link-to}} component does not disregard current-when when it is set via a bound param'](assert) {
    this.router.map(function () {
      this.route('index', {
        path: '/'
      }, function () {
  ...
  [`@test The {{link-to}} component supports 'target' attribute specified as a bound param`](assert) {
  ...
  [`@test the {{link-to}} component does not call preventDefault if 'preventDefault=false' is passed as an option`](assert) {
    this.router.map(function () {
      this.route('about');
    });
  ...
  [`@test the {{link-to}} component does not call preventDefault if 'preventDefault=boundFalseyThing' is passed as an option`](assert) {
    this.router.map(function () {
      this.route('about');
    });
  ...
  [`@test The {{link-to}} component does not call preventDefault if 'target' attribute is provided`](assert) {
    this.addTemplate('index', `
        <h3 class="home">Home</h3>
        {{#link-to 'index' id='self-link' target='_blank'}}Self{{/link-to}}
        `);
  ...
  [`@test The {{link-to}} component should not transition if target is not equal to _self or empty`](assert) {
    this.addTemplate('index', `
  ...
  [`@test The non-block form {{link-to}} component updates the link text when it is a binding`](assert) {
    assert.expect(8);
    this.router.map(function () {
      this.route('contact');
    });
  ...
  [`@test The non-block form {{link-to}} component moves into the named route with context`](assert) {
    assert.expect(5);
    this.router.map(function () {
      this.route('item', {
        path: '/item/:id'
  ...
  async [`@test the {{link-to}} component throws a useful error if you invoke it wrong`](assert) {
    ...
    await assert.rejects(this.visit('/'), /(You attempted to define a `\{\{link-to "post"\}\}` but did not pass the parameters required for generating its dynamic segments.|You must provide param `post_id` to `generate`)/);
  }
...
moduleFor('The {{link-to}} component - loading states and warnings', class extends ApplicationTestCase {
