...
if (EMBER_GLIMMER_ANGLE_BRACKET_BUILT_INS) {
  moduleFor('<LinkTo /> component with query-params (rendering)', class extends ApplicationTestCase {
    constructor() {
      super(...arguments);
    ...
    ['@test populates href with partially supplied query param values, but omits if value is default value']() {
      ...
      return this.visit('/').then(() => {
        this.assertComponentElement(this.firstChild, {
          ...
          attrs: {
            href: '/',
            class: classMatcher('ember-view active')
          },
          content: 'Index'
        });
      });
  ...
  moduleFor('<LinkTo /> component with query params (routing)', class extends ApplicationTestCase {
    constructor() {
      super();
      let indexProperties = {
        foo: '123',
    ...
    ['@test the <LinkTo /> component applies activeClass when query params are not changed'](assert) {
      this.addTemplate('index', `
    ...
    async ['@test The <LinkTo /> component disregards query-params in activeness computation when current-when is specified'](assert) {
      let appLink;
      this.router.map(function () {
        this.route('parent');
      });
