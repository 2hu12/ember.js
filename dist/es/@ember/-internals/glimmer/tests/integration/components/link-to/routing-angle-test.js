...
if (EMBER_GLIMMER_ANGLE_BRACKET_BUILT_INS) {
  ...
  moduleFor('<LinkTo /> component (routing tests)', class extends ApplicationTestCase {
    constructor() {
      super();
    ...
    [`@test the <LinkTo /> component doesn't apply a 'disabled' class if disabledWhen is not provided`](assert) {
    ...
    [`@test the <LinkTo /> component responds to clicks according to its disabledWhen bound param`](assert) {
    ...
    [`@skip The <LinkTo /> component supports 'classNameBindings' with custom values [GH #11699]`](assert) {
      this.addTemplate('index', `
          <h3 class="home">Home</h3>
  ...
  moduleFor('<LinkTo /> component (routing tests - location hooks)', class extends ApplicationTestCase {
    constructor() {
      super();
      this.updateCount = 0;
      this.replaceCount = 0;
    ...
    ['@test The <LinkTo /> component supports URL replacement via replace=boundTruthyThing'](assert) {
      this.addTemplate('index', `
          <h3 class="home">Home</h3>
  ...
  if (EMBER_IMPROVED_INSTRUMENTATION) {
    moduleFor('The <LinkTo /> component with EMBER_IMPROVED_INSTRUMENTATION', class extends ApplicationTestCase {
      constructor() {
        super();
        this.router.map(function () {
          this.route('about');
      ...
      ['@test The <LinkTo /> component interaction event includes the transition in the after hook'](assert) {
        assert.expect(1);
        subscribe('interaction.link-to', {
          before() {},
  ...
  moduleFor('The <LinkTo /> component - nested routes and link-to arguments', class extends ApplicationTestCase {
    ...
    [`@test The <LinkTo /> component does not disregard current-when when it is given explicitly for a route`](assert) {
      this.router.map(function () {
        this.route('index', {
          path: '/'
        }, function () {
    ...
    ['@test The <LinkTo /> component does not disregard current-when when it is set via a bound param'](assert) {
      this.router.map(function () {
        this.route('index', {
          path: '/'
        }, function () {
    ...
    [`@test The <LinkTo /> component supports 'target' attribute specified as a bound param`](assert) {
      this.addTemplate('index', `
          <h3 class="home">Home</h3>
    ...
    [`@test the <LinkTo /> component does not call preventDefault if '@preventDefault={{false}}' is passed as an option`](assert) {
      this.router.map(function () {
        this.route('about');
      });
      ...
      return this.visit('/').then(() => {
        assertNav({
          prevented: false
        }, () => this.$('#about-link').trigger('click'), assert);
      });
    }

    [`@test the <LinkTo /> component does not call preventDefault if '@preventDefault={{boundFalseyThing}}' is passed as an option`](assert) {
      this.router.map(function () {
        this.route('about');
      });
    ...
    [`@test The <LinkTo /> component does not call preventDefault if 'target' attribute is provided`](assert) {
      this.addTemplate('index', `
          <h3 class="home">Home</h3>
          <LinkTo id='self-link' @route='index' target='_blank'>Self</LinkTo>
          `);
    ...
    [`@test The <LinkTo /> component should not transition if target is not equal to _self or empty`](assert) {
      this.addTemplate('index', `
    ...
    [`@test The <LinkTo /> component works in an #each'd array of string route names`](assert) {
      ...
      let linksEqual = (links, expected) => {
        ...
        for (idx = 0; idx < links.length; idx++) {
          let href = this.$(links[idx]).attr('href'); // Old IE includes the whole hostname as well
    ...
    async [`@test the <LinkTo /> component throws a useful error if you invoke it wrong`](assert) {
      ...
      await assert.rejects(this.visit('/'), /(You attempted to generate a link for the "post" route, but did not pass the models required for generating its dynamic segments.|You must provide param `post_id` to `generate`)/);
    }
    ...
    [`@test the <LinkTo /> component's active property respects changing parent route context`](assert) {
      this.router.map(function () {
        this.route('things', {
          path: '/things/:name'
        }, function () {
    ...
    [`@test the <LinkTo /> component populates href with default query param values even without query-params object`](assert) {
      this.add('controller:index', Controller.extend({
        queryParams: ['foo'],
        foo: '123'
      }));
    ...
    [`@test the <LinkTo /> component populates href with default query param values with empty query-params object`](assert) {
      this.add('controller:index', Controller.extend({
        queryParams: ['foo'],
        foo: '123'
      }));
    ...
    [`@test the <LinkTo /> component with only query-params and a block updates when route changes`](assert) {
      this.router.map(function () {
        this.route('about');
      });
      this.add('controller:application', Controller.extend({
    ...
    [`@test GJ: <LinkTo /> to a parent root model hook which performs a 'transitionTo' has correct active class #13256`](assert) {
      assert.expect(1);
      this.router.map(function () {
  ...
  moduleFor('The <LinkTo /> component - loading states and warnings', class extends ApplicationTestCase {
